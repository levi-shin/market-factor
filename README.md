...
**GitHub Actions만 사용합니다.** AWS는 쓰지 않고, 데이터는 모두 이 저장소에 커밋됩니다.

## 구성

- `lambda_function.py` — 시세 수집, Gemini 분석, 로컬 JSON/HTML 저장, Slack 알림
- `index.html` — 마켓 팩터 웹앱 (대시보드 / 브리핑 / 리포트, `briefings.json` · `reports.json`)
...
| 주간 | 토 07:30~12:00 | `reports/YYYY-Www.html` |
| 월간 | 1일 07:30~12:00 | `reports/YYYY-MM-monthly.html` |
...
그래서 `daily.yml`은 **창(window) 안에서 약 30분 간격으로** 시도합니다.

| 세션 | 창 (KST) | 요일 | 슬롯 | Slack |
|------|----------|------|------|-------|
| 아침 | 07:30 ~ 12:00 | 월~토 | 10회 (07:33 ~ 11:33) | 발송 |
| 장마감 | 16:00 ~ 22:00 | 월~금 | 12회 (16:04 ~ 21:44) | 미발송 |

슬롯의 분(minute)은 `:00`/`:30`을 피해 흩어 놓았습니다. 매시 정각은 전 세계
워크플로가 몰리는 시간대라 GitHub가 실행을 뒤로 밀거나 통째로 흘립니다.
(2026-09-05~07 관측: 정시 크론이 8분~168분 지연, 절반 이상의 슬롯이 미발생)

크론 요일 필드는 UTC 기준이라 22~23시대 슬롯은 KST로 다음 날이 됩니다.
그래서 요일을 하루 앞당겨(`0-5`) 적었고, 덕분에 일요일과 토요일 장마감처럼
어차피 아무것도 하지 않는 시간대에는 러너가 아예 뜨지 않습니다.
...
창을 촘촘하게 깔아도 GitHub `schedule` 자체의 **지연**은 없앨 수 없습니다.
슬롯 누락은 메꿔지지만, 07:30 브리핑이 09:11에 도착하는 건 그대로입니다.
정시성이 필요하면 외부에서 `workflow_dispatch`를 호출하는 시계가 필요합니다.
(EventBridge로 구현했다가 운영 부담이 커서 걷어냈습니다 — `CHANGELOG.md` 0.7.0 참고)

당장 급하면 Actions → Daily Briefing → Run workflow로 수동 실행하면 됩니다.
